      ...
      <div className="max-w-2xl mx-auto p-4">
        ...
        {tab === 'progress' && (
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              {['Push', 'Pull', 'Legs'].map(day => (
                <button key={day} onClick={() => setProgressDay(day)}
                  className={`flex-1 py-2.5 rounded-xl text-sm font-medium transition-all flex items-center justify-center gap-2 ${progressDay === day ? 'bg-violet-600' : 'bg-zinc-900 text-zinc-400 hover:bg-zinc-800'}`}>
            ...
            {/* PRs for selected day */}
            <div className="bg-zinc-900 rounded-2xl p-4">
              <h3 className="font-semibold mb-3 flex items-center gap-2">
                <Trophy size={18} className="text-yellow-500" />PRs — {progressDay}
              </h3>
              <div className="grid grid-cols-2 gap-2">
                ...
                {Object.entries(prs).filter(([ex, _]) => exercises[progressDay]?.some(e => e.name === ex)).length === 0 && (
                  <div className="col-span-2 text-zinc-500 text-sm text-center py-2">No PRs yet</div>
                )}
              </div>
            </div>
            ...
            {/* No data message */}
            {history.filter(h => h.day === progressDay).length === 0 && (
              <div className="bg-zinc-900 rounded-2xl p-8 text-center">
                <div className="text-zinc-500 text-sm">No {progressDay} workouts logged yet</div>
                <div className="text-zinc-600 text-xs mt-1">Start logging to see your progress!</div>
              </div>
            )}
          </div>
        )}
